    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['system:academy:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['system:academy:remove']"
        >删除</el-button>
      </el-col>
      <!-- <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['system:academy:export']"
        >导出</el-button>
      </el-col> -->
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listAcademy(this.queryParams).then(response => {
        this.academyList = response.rows;
        this.total = response.total;
        this.loading = false;
      });

      listSchool({pageSize: 1000}).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除学院编号为"' + ids + '"的数据项？').then(function() {
        return delAcademy(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('system/academy/export', {
        ...this.queryParams
